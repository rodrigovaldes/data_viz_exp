...
How accurately are people able to assess the proportion out of a whole? Is it different their assessment in pies and waffles? If we state that the total number of respondents was 1000, the difference in means was statistically significant (10%), with a small effect size as indicated by a Cohen’s d value of 0.27. Then, people are better at estimating with waffles. 
...
We asked to sum two slices –areas– in the graph. Then, we compared that number with the data. We used two cases, one where the size of the slices was similar, and another where the size of the slices was evidently different.
